...
if ($name === null || $price === null || $description === null) {
      echo error("you probably forgot to send either the name, price or description of the servizio");
      exit;
}
...
if ($authData === null) {
      echo error("You must be authenticated to do add a servizio!");
      exit;
}

if ($authData["ruolo"] === "magazziniere" || $authData["ruolo"] === "admin") {
      $manager = new OfficineManager();
      $manager->addServizio($name, $price, $description);
      echo ok("servizio with name" . $name . " successfully added to all servizi");
} else {
